  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider<ShopProfileSetupUiCubit>(
          create: (context) => ShopProfileSetupUiCubit(),
        ),
      ],
          ...
          return BlocListener<ShopAuthBloc, ShopAuthState>(
            listener: (context, state) {
              final bool isSuccess =
                  state is ShopAuthSuccess ||
                  (state is ShopStatusLoaded &&
                      state.shop != null &&
                      state.shop!.isProfileCompleted &&
                      !state.shop!.isRejected);

              if (isSuccess && !uiState.isNavigated) {
                context.read<ShopProfileSetupUiCubit>().markNavigated();
